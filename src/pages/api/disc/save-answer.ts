...
export const POST: APIRoute = async ({ request, locals }) => {
  console.log("=========================================");
  console.log("🚀 [API DISC SAVE-ANSWER] Requisição iniciada!");
  ...
  try {
    ...
    const { assessment_id, question_id, disc_type } = data;

    // Valida campos obrigatórios
    if (!assessment_id || question_id === undefined || !disc_type) {
      console.error("❌ [API ERRO] Faltam campos obrigatórios:", { assessment_id, question_id, disc_type });
    ...
    // Valida que disc_type é D, I, S ou C
    if (!['D', 'I', 'S', 'C'].includes(String(disc_type).toUpperCase())) {
      console.error("❌ [API ERRO] disc_type inválido:", disc_type);
      return new Response(JSON.stringify({ error: "disc_type deve ser D, I, S ou C" }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' },
      });
    }
      ...
      .upsert(
        {
          assessment_id,
          question_id,
          disc_type: String(disc_type).toUpperCase(),
        },
