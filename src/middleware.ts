...
export const onRequest = defineMiddleware(async (context, next) => {
  ...
  const supabase = createServerClient(
    ...
    import.meta.env.PUBLIC_SUPABASE_ANON_KEY,
    {
      cookies: {
        getAll() {
          // Extrai o texto bruto do navegador e converte pro formato que o Supabase exige
  ...
  // 2. Validação REAL do token no servidor
  const { data: { user } } = await supabase.auth.getUser();
  locals.user = user ?? null;
  locals.supabase = supabase; // ← expõe o cliente SSR autenticado para todas as páginas
